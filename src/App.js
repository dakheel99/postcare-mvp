...
const DEPTS = ["الجراحة العامة","العظام","أمراض النساء","جراحة القلب","المسالك البولية","الجراحة العصبية","أمراض القلب","أخرى"];
    ...
    <div style={{ minHeight:"100vh", background:"#030712", color:"#f9fafb", fontFamily:"'Segoe UI',Tahoma,Arial,sans-serif", direction:"rtl", padding:"0 0 40px" }}>
      ...
      <div style={{ background:"#111827", borderBottom:"1px solid #1f2937", padding:"16px 24px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          ...
          <div>
            <div style={{ fontWeight:700, fontSize:15 }}>متابعة ما بعد العملية</div>
            <div style={{ fontSize:11, color:"#6b7280" }}>مبادرة أمان</div>
          </div>
        </div>
...
export default function App() {
  ...
  const openEdit = () => { setEditForm({name:sel.name,age:sel.age,phone:sel.phone,surgery:sel.surgery,dept:sel.dept,doctor:sel.doctor,risk:sel.risk,pain:sel.vitals.pain,fever:sel.vitals.fever,wound:sel.vitals.wound,mobility:sel.vitals.mobility,note:""}); setShowEdit(true); };
  const saveEdit = () => {
    ...
    <div style={{ minHeight:"100vh", background:"#030712", color:"#f9fafb", fontFamily:"'Segoe UI',Tahoma,Arial,sans-serif", direction:"rtl" }}>
      ...
      <div style={{ background:"#111827", borderBottom:"1px solid #1f2937", padding:"14px 24px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          ...
          <div><div style={{ fontWeight:700,fontSize:16 }}>رعاية ما بعد</div><div style={{ fontSize:11,color:"#6b7280" }}>منصة متابعة ما بعد العملية الجراحية</div></div>
        </div>
        <div style={{ display:"flex",alignItems:"center",gap:12 }}>
          ...
          <div style={{ display:"flex",alignItems:"center",gap:6,background:"#1f2937",borderRadius:10,padding:"6px 12px" }}><div style={{ width:8,height:8,borderRadius:"50%",background:"#10b981" }}></div><span style={{ fontSize:11,color:"#6b7280" }}>مبادرة أمان</span></div>
        </div>
      </div>
      ...
      <div style={{ padding:24, maxWidth:1200, margin:"0 auto" }}>
                    ...
                    <div style={{ padding:20 }}>
                      ...
                      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16 }}>
                        <div style={{ display:"flex",alignItems:"center",gap:14 }}>
                          ...
                          <div><div style={{ fontWeight:700,fontSize:18 }}>{sel.name}</div><div style={{ color:"#9ca3af",fontSize:13 }}>{sel.age} سنة • {sel.surgery}</div><div style={{ color:"#6b7280",fontSize:11 }}>{sel.doctor} — {sel.dept}</div><div style={{ color:"#6b7280",fontSize:11 }}>📱 {sel.phone}</div></div>
                        </div>
        ...
        {tab==="reports" && (
          <div style={card}>
            <div style={{ ...cardH,padding:20 }}><span style={{ fontWeight:700,fontSize:16 }}>📋 تقرير مبادرة أمان</span></div>
            <div style={{ padding:20 }}>
      ...
      {showAdd && (
        <div style={OVL}><div style={MDL}>
          ...
          <div style={R2}><div><label style={LBL}>الجوال *</label><input style={INP} placeholder="05xxxxxxxx" value={form.phone} onChange={e=>fc("phone",e.target.value)} /></div><div><label style={LBL}>درجة الخطورة</label><select style={INP} value={form.risk} onChange={e=>fc("risk",e.target.value)}><option value="منخفض">🟢 منخفض</option><option value="متوسط">🟡 متوسط</option><option value="مرتفع">🔴 مرتفع</option></select></div></div>
          <div style={{ marginBottom:12 }}><label style={LBL}>نوع العملية *</label><select style={INP} value={form.surgery} onChange={e=>fc("surgery",e.target.value)}><option value="">-- اختر --</option>{ALL_SURGERIES.map(s=><option key={s} value={s}>{s}</option>)}</select></div>
          <div style={R2}><div><label style={LBL}>القسم *</label><select style={INP} value={form.dept} onChange={e=>fc("dept",e.target.value)}><option value="">-- اختر --</option>{DEPTS.map(d=><option key={d} value={d}>{d}</option>)}</select></div><div><label style={LBL}>الطبيب *</label><input style={INP} placeholder="د. سعد الغامدي" value={form.doctor} onChange={e=>fc("doctor",e.target.value)} /></div></div>
      ...
      {showEdit && (
        <div style={OVL}><div style={MDL}>
          ...
          <div style={R2}><div><label style={LBL}>العملية</label><select style={INP} value={editForm.surgery||""} onChange={e=>ef("surgery",e.target.value)}>{ALL_SURGERIES.map(s=><option key={s} value={s}>{s}</option>)}</select></div><div><label style={LBL}>الخطورة</label><select style={INP} value={editForm.risk||"منخفض"} onChange={e=>ef("risk",e.target.value)}><option value="منخفض">🟢 منخفض</option><option value="متوسط">🟡 متوسط</option><option value="مرتفع">🔴 مرتفع</option></select></div></div>
